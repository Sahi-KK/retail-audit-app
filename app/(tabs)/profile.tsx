...
import { View, Text, TextInput, Pressable, ScrollView, Alert, KeyboardAvoidingView, Platform, Dimensions } from 'react-native';
import * as Updates from 'expo-updates';
import { useAuditStore } from '../../store/auditStore';
import { User, ShieldCheck, Save, LogOut, Globe } from 'lucide-react-native';
    ...
    >
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <View className="items-center mb-10 mt-6">
          ...
          <Text className="text-white text-2xl font-black">{auth.auditorName || 'Auditor Profile'}</Text>
        ...
        <View className="bg-white/5 p-8 rounded-[40px] border border-white/10 shadow-2xl">
          <View className="mb-6">
            <View className="flex-row items-center justify-between mb-2 ml-1">
              <Text className="text-[#C9A84C] text-[10px] font-black uppercase tracking-widest ">Full Name</Text>
              {auth.auditorId && <Globe size={12} color="#10B981" />}
            </View>
            {isEditing ? (
              <TextInput
                value={name}
          ...
          <View className="mb-10">
            <View className="flex-row items-center mb-2 ml-1">
              ...
              <Text className="text-[#C9A84C] text-[10px] font-black uppercase tracking-widest ml-2">Identity Hub ID</Text>
            </View>
            {isEditing ? (
              <TextInput
                value={id}
              ...
              <View className="bg-white/10 h-14 rounded-2xl px-5 border border-white/5 flex-row items-center justify-between">
                ...
                <View className="bg-[#C9A84C]/20 px-3 py-1 rounded-full border border-[#C9A84C]/30">
                  <Text className="text-[#C9A84C] text-[8px] font-black uppercase tracking-widest">Verified</Text>
                </View>
              </View>
            )}
          </View>
          ...
          {isEditing ? (
            ...
            >
              ...
              <Text className="text-rose-500 font-black text-lg ml-3">REVOKE ACCESS</Text>
            </Pressable>
          )}
        </View>
        ...
        <View className="mt-10">
            ...
            onPress={async () => {
              try {
                ...
                if (update.isAvailable) {
                  Alert.alert("Update Found", "A new version of the Identity Core is available. Restarting...");
                  await Updates.fetchUpdateAsync();
                  await Updates.reloadAsync();
                } else {
                  Alert.alert("Hub Synced", "Your environment is currently aligned with the Enterprise Master Hub.");
                }
              } catch (e) {
                Alert.alert("Offline", "Unable to reach the Master Hub. Local audits will be queued for later sync.");
              }
            }}
          ...
          >
            <View className="flex-row items-center">
              <View className="w-12 h-12 bg-emerald-500/10 rounded-2xl items-center justify-center mr-4">
                <ShieldCheck size={24} color="#10B981" />
              </View>
              <View>
                <Text className="text-white font-bold">Cloud Master Status</Text>
                <Text className="text-slate-500 text-[10px] uppercase font-black tracking-widest mt-1">Enterprise Core v2.1.0</Text>
              </View>
            </View>
        ...
        <View className="mt-10 p-8 bg-white/5 rounded-[40px] border border-white/10">
          <Text className="text-[#C9A84C] font-black text-[10px] uppercase tracking-[3px] mb-4 text-center">Security Protocol</Text>
          <Text className="text-slate-500 text-[11px] leading-5 text-center px-4">
            Audit attribution is final once synchronized to the Master Hub. Ensure your Auditor ID remains constant to maintain your diagnostic history.
          </Text>
          <Text className="text-white/10 text-center text-[7px] font-black uppercase tracking-[3px] mt-8">
            © 2026 ESSILORLUXOTTICA STRATEGIC ASSET
          </Text>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
