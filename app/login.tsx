...
import { View, Text, TextInput, Pressable, KeyboardAvoidingView, Platform, Alert, TouchableOpacity } from 'react-native';
...
export default function LoginScreen() {
  ...
  const validateAccess = (n: string, i: string) => {
    ...
    return ALLOWED_ACCOUNTS.some(acc => acc.name.toLowerCase() === trimmedName && acc.id === trimmedId);
  };
  ...
  const handleSignIn = () => {
    ...
    if (validateAccess(name, id)) {
      ...
      // Navigate to root layout which will direct to tabs
      router.replace('/');
    } else {
      notify(
        "Unauthorized Access", 
        "Please use your validated Enterprise credentials to access the Hub."
      );
    }
  };
    ...
    >
      <View className="flex-1 px-8 justify-center">
        ...
        <View className="bg-white/5 p-8 rounded-[40px] border border-white/10 shadow-2xl">
          ...
          <TouchableOpacity 
            onPress={() => {
              console.log("[DEBUG] ACCESS HUB clicked");
              handleSignIn();
            }}
            activeOpacity={0.8}
            className="bg-[#C9A84C] h-16 rounded-2xl flex-row items-center justify-center shadow-lg"
          >
